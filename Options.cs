...
namespace ArmenDiplom
{
  public partial class Options : Form
  {
    ...
    private void Options_Load(object sender, EventArgs e)
    {
      ...
      catch (SystemException)
      {
        MessageBox.Show("Введены неправильные значения в конфигурационном файле ArmenDiplom.exe.config." + 
          "\n попробуйте исправить их. Программа будет закрыта", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
        Application.Exit();
      }
    }
    ...
    private void endDayUpDown_Validating(object sender, CancelEventArgs e)
    {
      ...
      {
        optionsErrorProvider.SetError((Control)sender, "Конечный час должен быть больще начального часа");
      }
