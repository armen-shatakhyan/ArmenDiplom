...
namespace ArmenDiplom
{
  public partial class ChangeCounter : Form
  {
    ...
    public ChangeCounter()
    {
      ...
      {
        var query = (from f in DataBaseAccess.db.Shkafs
                    where f.Counters.Count < f.CountersQuantity
                    select f).ToArray();
        shkafComboBox.Items.AddRange(query);
      }
    ...
    private void counterNumberTextBox_Validating(object sender, CancelEventArgs e)
    {
      try
      {
        if (counterNumberTextBox.Text.Trim().Length != 7)
        {
          throw new Exception();
        }
      ...
      {
        errorNewShkaf.SetIconAlignment((Control)sender, ErrorIconAlignment.MiddleRight);
        errorNewShkaf.SetError((Control)sender, "Значение должно быть 7-и значное число");
      }
    }
