...
{
  public partial class BadAccess : Form
  {
    private Dictionary<int, string> months;
    public static bool isOpened = false;
    private IQueryable<BadAccessR> query;
    public BadAccess()
    {
      InitializeComponent();
      isOpened = true;
    ...
    private void BadAccess_FormClosed(object sender, FormClosedEventArgs e)
    {
      isOpened = false;
    }
    ...
    private void button1_Click(object sender, EventArgs e)
    {
      ...
      else
      {
        ...
        dataTable.Columns.Add("Количество допусков", typeof(int));
        dataTable.Columns.Add("Количество несанкционированных допусков", typeof(int));

        if (!dateCheckbox.Checked)
        {
           query = from f in DataBaseAccess.db.ShkafStatements
                       where f.BadAccessQuantity > 0
                      select new BadAccessR
                      {
                        ShkafID = f.ShkafID,
                        Address = f.Shkaf.Address,
                        Year = f.Year,
                        Month = months[f.Month],
                        AccessQuantity = f.AccessQuantity,
                        BadAccessQuantity = f.BadAccessQuantity
                      };
          foreach (var q in query)
          {
            dataTable.Rows.Add(new object[] { q.ShkafID, q.Address, q.Year, q.Month, q.AccessQuantity, q.BadAccessQuantity });
          }
        }
        ...
        {
           query = from f in DataBaseAccess.db.ShkafStatements
                  where f.Year == beginDateTimePicker.Value.Year
        ...
        dataGridView1.Columns["Адрес"].Width = 200;
        dataGridView1.Columns["Количество допусков"].Width = 130;
        dataGridView1.Columns["Количество несанкционированных допусков"].Width = 200;
      }
    }
    ...
    private void button2_Click(object sender, EventArgs e)
    {
      ...
      {
        var a = query.ToList();
        if (!BadAccessReportForm.isOpened)
        {
          BadAccessReportForm badAccessReportForm = new BadAccessReportForm(a, beginDateTimePicker.Value.ToShortDateString(), 
            endDateTimePicker.Value.ToShortDateString(), dateCheckbox.Checked);
          badAccessReportForm.Show();
        }
      }
    }
  }

  public class BadAccessR
  {
    public int ShkafID { get; set; }
    public string Address { get; set; }
    public int Year { get; set; }
    public string Month { get; set; }
    public int AccessQuantity { get; set; }
    public int BadAccessQuantity { get; set; }
  }
}
